...
use graphics::{batched_renderer2d::BatchedRenderer2D, buffers::*, renderable2d::Renderable2D, renderer::Render, shader::Shader, simple2d_renderer::Simple2dRenderer, sprite::Sprite, static_sprite::StaticSprite, window::Window};
use mat4::Mat4;
use math::*;
use vec2::Vec2;
use vec3::Vec3;
...
fn main() {
    ...
    let renderable = Renderable2D::from(Vec3::new(5.0, 5.0, 0.0), Vec2::new(4.0, 4.0), Vec4::new(1.0, 0.4, 0.45, 1.0));
    let sprite_1 = Sprite::from(renderable);
    let renderable = Renderable2D::from(Vec3::new(0.0, 0.0, 0.0), Vec2::new(2.0, 2.0), Vec4::new(0.1, 0.4, 0.45, 1.0));
    let sprite_2 = Sprite::from(renderable);


    let mut simple_renderer = Simple2dRenderer::new();
    let renderable = Renderable2D::from(Vec3::new(1.0, 1.0, 0.0), Vec2::new(2.0, 2.0), Vec4::new(0.1, 0.4, 0.45, 1.0));
