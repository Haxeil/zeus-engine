...
use crate::utils::file_utils::opengl_error;
use crate::vec3::Vec3;
...
impl BatchedRenderer2D {
    pub fn new() -> Self {
        ...
        unsafe {
            ...
            gl::VertexAttribPointer(SHADER_VERTEX_INDEX, 3, gl::FLOAT, gl::FALSE, stride, 0 as *const _);
            gl::VertexAttribPointer(SHADER_COLOR_INDEX, 4, gl::FLOAT, gl::FALSE, stride, (3 * size_of::<GLfloat>()) as *const GLvoid);
    ...
    where T: Renderable
{
    fn submit(&mut self, sprite: &T) {
        ...
        println!("Submitting sprite with position: {:?}, size: {:?}, color: {:?}", position, size, color);
        unsafe {
            ...
            println!("Vertex 1: Position: {:?}, Color: {:?}", (*vertex_base).vertex, (*vertex_base).color);
        
            // Second Vertex
            (*vertex_base.add(1)).vertex = Vec3::new(position.x, position.y + size.y, position.z);
            (*vertex_base.add(1)).color = color;
            println!("Vertex 2: Position: {:?}, Color: {:?}", (*vertex_base.add(1)).vertex, (*vertex_base.add(1)).color);
        
            // Third Vertex
            (*vertex_base.add(2)).vertex = Vec3::new(position.x + size.x, position.y + size.y, position.z);
            (*vertex_base.add(2)).color = color;
            println!("Vertex 3: Position: {:?}, Color: {:?}", (*vertex_base.add(2)).vertex, (*vertex_base.add(2)).color);
        
            // Fourth Vertex
            (*vertex_base.add(3)).vertex = Vec3::new(position.x + size.x, position.y, position.z);
            (*vertex_base.add(3)).color = color;
            println!("Vertex 4: Position: {:?}, Color: {:?}", (*vertex_base.add(3)).vertex, (*vertex_base.add(3)).color);
    ...
    fn flush(&mut self) {
        unsafe {
            ...
            // Draw the elements
            gl::DrawElements(gl::TRIANGLES, self.index_count as i32, gl::UNSIGNED_SHORT, null());
            opengl_error();
